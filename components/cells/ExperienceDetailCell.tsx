...
export default function ExperienceDetailCell({
  ...
  return (
    <Cell cellNumber={cellNumber} timestamp={timestamp} duration={"<1s"} executed>
      ...
      <div className="rounded-r-lg border-l-[3px] border-[#FF3621]/70 bg-db-gray-50/30 py-4 pl-4 pr-3 dark:bg-db-gray-800/20">
        {/* Header with logo */}
        <div className="flex items-start gap-4">
          {logo && (
            <div className="relative mt-0.5 h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-db-border bg-white shadow dark:bg-db-gray-100">
              <Image
                src={logo}
                alt={company}
                fill
          ...
          <div className="min-w-0">
            <h3 className="text-lg font-bold tracking-tight text-db-gray-900 dark:text-db-gray-100">
              {role}
            </h3>
            <p className="mt-0.5 text-[15px]">
              <span className="font-semibold text-db-gray-700 dark:text-db-gray-300">{company}</span>
              <span className="mx-1.5 text-db-gray-400">|</span>
              <span className="text-db-gray-500 dark:text-db-gray-400">{period}</span>
            </p>
            <p className="mt-1 inline-flex items-center gap-1.5 text-[13px] text-db-gray-500 dark:text-db-gray-400">
        ...
        {/* Contributions count */}
        <p className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-db-gray-500 dark:text-db-gray-400">
          Key Contributions
        ...
        <ul className="space-y-3 pl-0">
          {visibleBullets.map((b, i) => (
            <li
              key={i}
              className="flex items-start gap-2.5 text-[14px] leading-[1.7] text-db-gray-700 dark:text-db-gray-300"
            >
